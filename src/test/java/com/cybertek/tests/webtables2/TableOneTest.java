...
public class TableOneTest extends TestBase {
    ...
    public int[] getTableSize() {
        ...
        return new int [] {getRowCount(),getColCount()};
    }
    ...
    public List<WebElement> getCellsInColumn(String colName) {
        ...
        for (int i=0; i<getHeaders().size(); i++){
            int ind=BrowserUtils.getElementsText(getHeaders()).indexOf(colName);
            if(getHeaders().get(i).getText().equals(colName)){
                index=i+1;
            }
        }
        return driver.findElements(By.xpath("//table[@id='table1']/tbody/tr/td["+index+"]"));
    }
